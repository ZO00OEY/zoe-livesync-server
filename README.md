...
- 默认不停止、不改写、不接管服务器上已有的代理节点；只有用户明确输入 `YES` 后，才会在证书验证期间临时停止已识别的 80 端口服务并自动恢复。
...
如果 TCP 80 被占用，脚本会先检查 443 当前提供的证书，再在常见证书目录和占用 443 的 Docker 容器挂载中查找匹配私钥。只有域名指向本机公网 IP、证书有效且私钥匹配时才会建议复用；找不到时才进入下述占用者确认。复用模式不会接管证书续签，只会定时同步更新后的证书并重载本项目 Caddy。

如果没有可复用证书，脚本会继续识别 80 端口占用者。占用者明确属于单个 Docker 容器或 systemd 服务时，会用红色危险提示显示名称，并要求用户完整输入 `YES` 授权。获得授权后，首次签发及真正需要续签时才会：

1. 再次确认占用者与上次授权对象一致；
2. 通过 Docker 或 systemd 临时停止它；
3. 运行 acme.sh 的 HTTP-01 临时验证服务；
4. 验证结束后无论成功、失败或中断都尝试恢复原服务。

无法归属到 Docker/systemd、同时存在多个占用者或占用对象发生变化时，脚本只显示进程信息并退出，不会直接 `kill` PID。自动续签在证书尚未进入续签窗口时不会停止原服务。
