...
certificate_covers_public_host() {
    if valid_ipv4 "${PUBLIC_HOST}"; then
        openssl x509 -checkip "${PUBLIC_HOST}" -noout -in "$1" >/dev/null 2>&1
    else
        openssl x509 -checkhost "${PUBLIC_HOST}" -noout -in "$1" >/dev/null 2>&1
    fi
}

public_host_points_here() {
    if valid_ipv4 "${PUBLIC_HOST}"; then
        is_public_ipv4 "${PUBLIC_HOST}" && [[ "${PUBLIC_HOST}" == "${PUBLIC_IP}" ]]
    else
        valid_hostname "${PUBLIC_HOST}" && hostname_points_here "${PUBLIC_HOST}"
    fi
}
...
validate_existing_certificate() {
    ...
    public_host_points_here || die "PUBLIC_HOST 必须是当前公网 IP，或解析到该 IP 的有效域名：${PUBLIC_HOST}"
    openssl x509 -checkend 86400 -noout -in "${TLS_CERT_FILE}" >/dev/null 2>&1 || die "现有证书无效或将在 24 小时内过期。"
    certificate_covers_public_host "${TLS_CERT_FILE}" || die "现有证书不包含 ${PUBLIC_HOST}。"
...
select_tls_mode() {
    ...
    ok "HTTPS 模式：$([[ "${TLS_MODE}" == "existing" ]] && printf '复用现有证书' || printf '新签发公网 IP 证书')；访问地址 ${PUBLIC_URL}"
}
...
issue_certificate() {
    ...
    if managed_ip_certificate_is_valid; then
        ...
        if port_is_busy 80; then
            ss -H -ltnp 'sport = :80' 2>/dev/null >&2 || true
            die "签发公网 IP 证书需要 TCP 80，但该端口已被占用；脚本不会停止现有服务。"
        fi
